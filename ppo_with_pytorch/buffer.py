...
class Buffer:
    ...
    def initialise_buffer(self):
        ...
        self.buffer = {
            ...
            # 'infos': np.zeros((self.NUM_AGENTS, self.rollout_length)),
            }
    ...
    def reset(self):
        ...
        del self.buffer  # Remove the reference to the buffer
        gc.collect() 
    ...
    def evaluate_gae_advantages(self):
        ...
        for agent_i in range(self.NUM_AGENTS): 
            ...
            self._normalise_agent_data('advantages', agent_i)
    ...
    def _evaluate_agent_advantage(self, agent_i):
        ...
        for t in range(self.rollout_length): 
            ...
            return_one_step = self.buffer['returns'][agent_i][t]
            pred_value = self.buffer['pred_values'][agent_i][t]
                
            self.buffer['advantages'][agent_i][t] = return_one_step - pred_value

    def evaluate_returns(self):
        for agent_i in range(self.NUM_AGENTS): 
            self._evaluate_agent_return(agent_i)

    def _evaluate_agent_return(self, agent_i): 
        '''calculates returns assuming rewards has already been calculated, r + (lamda * r+1)
        currently does 1 step TODO: N-STEPS'''
        rewards = self.buffer['rewards'][agent_i]

        for t in reversed(range(self.rollout_length)): # l=10, t is 9 8 7 6 
            
            if t < self.rollout_length - 1:
                self.buffer['returns'][agent_i][t] = rewards[t] + (self.discount * rewards[t+1])

            else: # if t == last index 
                self.buffer['returns'][agent_i][t] = rewards[t]
...
class EpisodeBuffer:
    ...
    def reset(self): 
        ...
        del self.all_episodes_buffer  # Remove the reference to the buffer
        gc.collect() 
    ...
    def _initialise_episode_buffer(self):
        ...
        'advantages': np.zeros((self.NUM_AGENTS, self.NUM_EPISODES, self.ROLLOUT_LENGTH)),  # returns - pred_values
        'action_probs': np.zeros((self.NUM_AGENTS, self.NUM_EPISODES, self.ROLLOUT_LENGTH)),
        } # TODO: reduce repeated code, pass in buffer to this class to then create a larger episodic buffer. 

        return self.all_episodes_buffer 


    def flatten_data(self, agent_i): 
        '''Slides data for agent, then flattens it across (self.NUM_EPISODES, self.ROLLOUT_LENGTH), ready for ppo updates
        TODO: flatten according to keys specified, think about how outer world interacts with class'''
        ...
        agent_action_probs = self.all_episodes_buffer['action_probs'][agent_i].reshape(-1) # (no_epi, time_steps, dim) -> (no_epi*timesteps, dim)
        agent_advantages = self.all_episodes_buffer['advantages'][agent_i].reshape(-1)

        return (agent_obs, agent_pred_values, agent_action_probs, agent_advantages)
    ...
    def get_state(self):
        return deepcopy(self.all_episodes_buffer)
