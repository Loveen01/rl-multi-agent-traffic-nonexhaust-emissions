...
class CountAllRewardsEnvPZ(SumoEnvironmentPZ):
    ...
    def __init__(self, **kwargs):
        ...
        self.seed()
        self.env = CountAllRewardsEnv(**self._kwargs)  # instead of SumoEnvironment. CountAllRewardsEnv is just a variant of 

        self.agents = self.env.ts_ids  # you dont actually need to redefine all the attributes, they are already inherited from SumoEnvPz.
...
# this class inherits the reward class defined above, however it adds some other features. 
# holds variables for max_dist, controlled_lanes, 
# it overrides the following functions: _apply_actions, _compute_info, action_space, _sumo_step, close()
# keeps a direct. pathway to store values 
# if eval, it measures system stats, agent stats, appends to the csv file. does this in sumo_step()
# this should be called Single agent environment
class MultiAgentSumoEnv(CountAllRewardsEnv):
    ...
    def _apply_actions(self, actions): # not sure why this sets action to zero if the action is second last on list?
        ...
        """
        for ts, action in actions.items(): # iterate through actions for each traffic signal
            if self.traffic_signals[ts].time_to_act:  
                n = self.action_spaces(ts).n # n = numb of elements in action space 
                if action > n - 1:           # set the action to 0 
                    action = 0
...
class RealMultiAgentSumoEnv(SumoEnvironmentPZ):
    '''Difference between this and the above is that this accesses everything through the self.env unlike the above. '''
    ...
    def _apply_actions(self, actions): # not sure why this sets action to zero if the action is second last on list?
        ...
        for ts, action in actions.items(): # iterate through actions for each traffic signal
            if self.env.traffic_signals[ts].time_to_act:  
                ...
                if action > n - 1:           # set the action to 0 
                    action = 0
                self.env.traffic_signals[ts].set_next_phase(action)
    ...
    def _compute_info(self):
        ...
        for agent_id in self.ts_ids:
            ...
            # Add tyre PM
            agent_info["tyre_pm"] = get_tyre_pm(self.env.traffic_signals[agent_id])
    ...
    def _sumo_step(self):
        ...
        if self.eval:
            # SYSTEM
            vehicles = self.env.sumo.vehicle.getIDList() # get all vehicles in env
            speeds = [self.env.sumo.vehicle.getSpeed(veh) for veh in vehicles] # [speedv1 speedv2]
            waiting_times = [self.env.sumo.vehicle.getWaitingTime(veh) for veh in vehicles] # [waitv1 waitv2]

            system_tyre_pm = get_tyre_pm()  # lo. tyre emmissions for all vehicles in simulation...
            arrived_num = self.env.sumo.simulation.getArrivedNumber() # 
            self.tyre_pm_system += system_tyre_pm # lo. append to cumulative...
            self.arrived_so_far += arrived_num # lo. append to cumulative counter...
            ...
            agent_speeds = [self.env.sumo.vehicle.getSpeed(veh) for veh in observable_vehs]
            agent_waiting_times = [self.env.sumo.vehicle.getWaitingTime(veh) for veh in observable_vehs]
            agent_accum_waits = [self.env.sumo.vehicle.getAccumulatedWaitingTime(veh) for veh in observable_vehs]
            
            agents_tyre_pm = sum(abs(self.env.sumo.vehicle.getAcceleration(veh)) for veh in observable_vehs)
            self.tyre_pm_agents += agents_tyre_pm
    ...
    def get_observable_vehs(self, lane) -> List[str]:
        """Remove undetectable vehicles from a lane."""
        detectable = []
        for vehicle in self.env.sumo.lane.getLastStepVehicleIDs(lane): # Returns the ids of the vehicles for the last time step on the given lane
            path = self.env.sumo.vehicle.getNextTLS(vehicle) # Return list of upcoming traffic lights [(tlsID, tlsIndex, distance, state), ...]
            if len(path) > 0:
                next_light = path[0]
                distance = next_light[2]
                if distance <= self.max_dist:  # Detectors have a max range. if distance from vehicle to traffic light is < 200, vehicle is detectable from traffic light
